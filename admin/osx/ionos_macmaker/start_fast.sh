...
# This script is a fast dev-loop variant of start.sh:
# - only builds the "nextcloud" target (no install/package/sign step)
# - skips the CMake reconfigure step unless a clean rebuild is requested
# or no CMakeCache.txt exists yet
set -xe
...
while getopts "a:b:s:cifoumr" opt; do
  case ${opt} in
    ...
    \? )
      echo "Usage: start_fast.sh -a <architecture> -b <build_dir> [-s <code_sign_identity>] [-c] [-f] [-i] [-m] [-o] [-r] [-u]"
      exit 1
      ;;
  esac
done
...
# Only build the nextcloud target itself (no install/package/sign step).
# Mirrors what CLion does with `cmake --build ... --target nextcloud`.
ninja -C $BUILD_DIR nextcloud -v
