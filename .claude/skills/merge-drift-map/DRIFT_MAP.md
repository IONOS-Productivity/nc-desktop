...
| `src/gui/folderstatusdelegate.cpp` | ja | ~74% (359/482) | **nicht vollständig geprüft** (Ausnahme: `backupStyle`/`Q_OS_MACOS`-Guard gezielt geprüft, s. Bemerkung) | gemischt | `1d8a6b4eb` / `1f67d9d4c` | Gezielt geprüft: `Q_OS_MACOS`-Guard um `backupStyle` (Fusion-Style für Sync-Progressbar) entfernt, Commit `83c171240` (SES-606) — **bewusste Abweichung**, stable-33.0 führt den Guard unverändert (Upstream-Commits `ac0d39e21`/`d31a9ec9c`, reiner `QMacStyle`-Bugfix-Workaround für macOS), Fork nutzt Fusion jetzt für alle Plattformen für einheitliches Whitelabel-Look, siehe `DECISIONS.md` 2026-09-22 — **niedrig/mittel Risiko** (isolierte, gut lokalisierte Stelle, aber stable ändert genau diesen Guard erfahrungsgemäß öfter). Rest der Datei weiterhin nicht vollständig geprüft (u. a. neue Commits SES-618 "progress bar green and thicker", SES-613 seit letzter Vollprüfung) — Kandidat für eine vertiefte `stable-merge-check`-Runde |
...
- **hoch Diff-Anteil + niedriges/mittleres Risiko** (häufige, aber überwiegend harmlose Konflikte zu erwarten): `folderstatusdelegate.cpp` (74%, nur Teilbereich `backupStyle`-Guard qualitativ geprüft — niedrig/mittel), `ignorelisttablewidget.cpp` (76%), `folderwizard.cpp` (63%), `legacyaccountselectiondialog.cpp` (61%), `Style.qml` (60%).
